...
function verifyToken(token) {
  try {
    ...
    const expected = (0, import_crypto.createHmac)("sha256", SECRET).update(data).digest("base64url");
...
async function handler(req, res) {
  try {
    ...
    const rows = await db.select().from(users).where((0, import_drizzle_orm2.eq)(users.id, payload.userId));
    if (rows.length === 0) {
